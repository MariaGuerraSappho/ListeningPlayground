...
class SoundExplorer {
        ...
        this.missions = [
            ...
            },
            {
                ...
                check: (chars) => {
                    const avgHigh = chars.reduce((sum, c) => sum + c.highAvg, 0) / chars.length;
                    const avgLow = chars.reduce((sum, c) => sum + c.lowAvg, 0) / chars.length;
                    return avgHigh > 20 && avgHigh > avgLow * 1.05;
                }
            }
        ];
            ...
            <div class="sound-card" data-id="${rec.id}">
                ...
                <div class="sound-card-content">
                    ...
                    <div class="sound-card-controls">
                        ...
                        <button class="btn-secondary btn-delete" data-id="${rec.id}">Delete</button>
                    </div>
                </div>
            </div>
        `).join('');
    ...
    async playRecording(id) {
        ...
        if (btn) { btn.classList.add('playing'); btn.innerHTML = '<span>Playing</span>'; }
        audio.play();
        audio.onended = () => { this.activeAudios.delete(audio); if (btn) { btn.classList.remove('playing'); btn.innerHTML = '<span>Play</span>'; } this.currentAudio = null; };
    }
    ...
    renderMap() {
        ...
        bubbles.innerHTML = all.map(rec => {
            ...
            return `<div class="sound-bubble" data-id="${rec.id}" title="${rec.label}">
              ...
              <button class="bubble-add-btn" data-id="${rec.id}" aria-label="Add to map">Add</button>
    ...
    renderMissions() {
        ...
        statsDiv.innerHTML = `
            <div>Badges: ${completed} / ${total}</div>
        `;
